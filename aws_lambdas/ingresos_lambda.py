import json
import pyodbc 
import hmac
import hashlib
import base64
import time
from decimal import Decimal

class CustomEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, Decimal):
            return float(obj)
        if hasattr(obj, 'isoformat'):
            return obj.isoformat()
        return super(CustomEncoder, self).default(obj)

def verify_jwt(auth_header):
    if not auth_header or not auth_header.startswith("Bearer "):
        return None
    token = auth_header.split(" ")[1]
    try:
        header_b64, payload_b64, signature_b64 = token.split(".")
        secret = "secreto123"
        expected_sig = base64.urlsafe_b64encode(hmac.new(secret.encode(), f"{header_b64}.{payload_b64}".encode(), hashlib.sha256).digest()).decode().rstrip('=')
        if signature_b64 != expected_sig:
            return None
        payload_b64 += "=" * ((4 - len(payload_b64) % 4) % 4)
        payload = json.loads(base64.urlsafe_b64decode(payload_b64).decode())
        if payload.get("exp") < int(time.time()):
            return None
        return payload
    except Exception:
        return None

def get_connection():
    return pyodbc.connect(
        "Driver={ODBC Driver 18 for SQL Server};"
        "Server=dbweb.cyiiinurskzo.us-east-1.rds.amazonaws.com;"
        "Database=TaxFlow;"
        "UID=admin;"
        "PWD=Macarron1302;"
        "TrustServerCertificate=yes;"
    )

def lambda_handler(event, context):
    headers = event.get('headers', {})
    auth_header = headers.get('Authorization') or headers.get('authorization')
    
    # Extraer el método HTTP (Soporta Payload v1.0 y v2.0)
    http_method = event.get('httpMethod')
    if not http_method and 'requestContext' in event and 'http' in event['requestContext']:
        http_method = event['requestContext']['http'].get('method')
    if not http_method:
        http_method = 'GET'



    user = verify_jwt(auth_header)
    if not user:
        return {"statusCode": 401, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": "No autorizado"})}
        
    usuario_id = user["usuario_id"]
    path_parameters = event.get('pathParameters') or {}
    ingreso_id = path_parameters.get('id')
    
    conn = None
    cursor = None
    
    try:
        conn = get_connection()
        cursor = conn.cursor()
        
        if http_method == 'GET':
            if ingreso_id:
                cursor.execute("""
                    SELECT i.id, i.monto, i.fecha_ingreso, i.descripcion, i.retencion_aplicada,
                           ti.nombre AS tipo_ingreso, m.codigo AS moneda_codigo, i.tipo_ingreso_id, i.moneda_id
                    FROM ingresos i
                    JOIN tipos_ingreso ti ON ti.id = i.tipo_ingreso_id
                    JOIN monedas m ON m.id = i.moneda_id
                    WHERE i.id = ? AND i.usuario_id = ?
                """, ingreso_id, usuario_id)
                row = cursor.fetchone()
                if not row:
                    return {"statusCode": 404, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": "No encontrado"})}
                
                columns = [column[0] for column in cursor.description]
                return {"statusCode": 200, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "success", "data": dict(zip(columns, row))}, cls=CustomEncoder)}
            else:
                cursor.execute("""
                    SELECT i.id, i.monto, i.fecha_ingreso, i.descripcion, i.retencion_aplicada,
                           ti.nombre AS tipo_ingreso, m.codigo AS moneda_codigo
                    FROM ingresos i
                    JOIN tipos_ingreso ti ON ti.id = i.tipo_ingreso_id
                    JOIN monedas m ON m.id = i.moneda_id
                    WHERE i.usuario_id = ?
                    ORDER BY i.fecha_ingreso DESC
                """, usuario_id)
                columns = [column[0] for column in cursor.description]
                results = [dict(zip(columns, row)) for row in cursor.fetchall()]
                return {"statusCode": 200, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "success", "data": results}, cls=CustomEncoder)}
                
        elif http_method == 'POST':
            body = json.loads(event.get('body', '{}'))
            action = body.get('action')

            if action == 'delete' and ingreso_id:
                cursor.execute("DELETE FROM ingresos WHERE id = ? AND usuario_id = ?", ingreso_id, usuario_id)
                if cursor.rowcount == 0:
                    return {"statusCode": 404, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": "No encontrado"})}
                conn.commit()
                return {"statusCode": 200, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "success", "message": "Eliminado"})}

            elif action == 'update' and ingreso_id:
                cursor.execute("""
                    UPDATE ingresos
                    SET monto = ?, fecha_ingreso = ?, descripcion = ?, retencion_aplicada = ?, tipo_ingreso_id = COALESCE(?, tipo_ingreso_id), moneda_id = COALESCE(?, moneda_id), fecha_actualizacion = GETDATE()
                    WHERE id = ? AND usuario_id = ?
                """, body.get('monto'), body.get('fecha_ingreso'), body.get('descripcion'), body.get('retencion_aplicada', 0), body.get('tipo_ingreso_id'), body.get('moneda_id'), ingreso_id, usuario_id)
                if cursor.rowcount == 0:
                    return {"statusCode": 404, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": "No encontrado"})}
                conn.commit()
                return {"statusCode": 200, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "success", "message": "Actualizado"})}

            else:
                cursor.execute("""
                    INSERT INTO ingresos (usuario_id, tipo_ingreso_id, moneda_id, monto, fecha_ingreso, descripcion, retencion_aplicada)
                    OUTPUT INSERTED.id
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, usuario_id, body.get('tipo_ingreso_id'), body.get('moneda_id'), body.get('monto'), body.get('fecha_ingreso'), body.get('descripcion'), body.get('retencion_aplicada', 0))
                new_id = cursor.fetchone()[0]
                conn.commit()
                return {"statusCode": 201, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "success", "data": {"id": new_id}})}

    except Exception as e:
        return {"statusCode": 500, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": str(e)})}
    finally:
        if cursor: cursor.close()
        if conn: conn.close()
