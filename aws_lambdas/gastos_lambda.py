...
def lambda_handler(event, context):
    ...
    gasto_id = path_parameters.get('id')
    ...
    try:
        ...
        if http_method == 'GET':
            if gasto_id:
                cursor.execute("""
                    SELECT g.id, g.monto_total, g.monto_deducible, g.fecha_gasto, g.estado,
                           cg.nombre AS categoria, g.categoria_gasto_id, g.comprobante_id
                    FROM gastos_deducibles g
            ...
            if action == 'delete' and gasto_id:
                cursor.execute("DELETE FROM gastos_deducibles WHERE id = ? AND usuario_id = ?", gasto_id, usuario_id)
                if cursor.rowcount == 0:
                    return {"statusCode": 404, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": "No encontrado"})}
                conn.commit()
                return {"statusCode": 200, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "success", "message": "Eliminado"})}

            elif action == 'update' and gasto_id:
                cursor.execute("""
                    UPDATE gastos_deducibles
                    SET monto_total = ?, monto_deducible = ?, fecha_gasto = ?, categoria_gasto_id = COALESCE(?, categoria_gasto_id), fecha_actualizacion = GETDATE()
                    WHERE id = ? AND usuario_id = ?
                """, body.get('monto_total'), body.get('monto_deducible'), body.get('fecha_gasto'), body.get('categoria_gasto_id'), gasto_id, usuario_id)
                if cursor.rowcount == 0:
                    return {"statusCode": 404, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "error", "message": "No encontrado"})}
                conn.commit()
                ...
                cursor.execute("""
                    INSERT INTO gastos_deducibles (usuario_id, categoria_gasto_id, monto_total, monto_deducible, fecha_gasto, estado)
                    OUTPUT INSERTED.id
                    VALUES (?, ?, ?, ?, ?, 'PENDIENTE')
                """, usuario_id, body.get('categoria_gasto_id'), body.get('monto_total'), body.get('monto_deducible', 0), body.get('fecha_gasto'))
